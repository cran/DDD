dd_KI_simplex = function(trparsopt,trparsfix,idparsopt,idparsfix,idparsnoshift,pars2,brtsM,brtsS,missnumspec)
{
...
for(i in 1:(numpar + 1))
{
   fv[i] = -dd_KI_loglik_choosepar(v[,i],trparsfix,idparsopt,idparsfix,idparsnoshift,pars2,brtsM,brtsS,missnumspec)
}
...
{ 
   ...
   fxr = -dd_KI_loglik_choosepar(xr,trparsfix,idparsopt,idparsfix,idparsnoshift,pars2,brtsM,brtsS,missnumspec)
 
   if(fxr < fv[1])
   {
       ## Calculate expansion point
       xe = (1 + rh * ch) * xbar - rh * ch * v[,numpar + 1]
       fxe = -dd_KI_loglik_choosepar(xe,trparsfix,idparsopt,idparsfix,idparsnoshift,pars2,brtsM,brtsS,missnumspec)
       if(fxe < fxr)
       {
           v[,numpar + 1] = xe
           fv[numpar + 1] = fxe
       ...
       {      
           ...
           if(fxr < fv[numpar + 1])
           {
              ...
              fxco = -dd_KI_loglik_choosepar(xco,trparsfix,idparsopt,idparsfix,idparsnoshift,pars2,brtsM,brtsS,missnumspec)
              if(fxco <= fxr)
              {
                 v[,numpar + 1] = xco
                 fv[numpar + 1] = fxco            
              ...
              fxci = -dd_KI_loglik_choosepar(xci,trparsfix,idparsopt,idparsfix,idparsnoshift,pars2,brtsM,brtsS,missnumspec)
              if(fxci < fv[numpar + 1])
              {  
                 v[,numpar + 1] = xci
                 fv[numpar + 1] = fxci
           ...
           if(how == "shrink")
           {
               for(j in 2:(numpar + 1))
               {
                   ...
                   v[,j] = v[,1] + si * (v[,j] - v[,1])
                   fv[j] = -dd_KI_loglik_choosepar(v[,j],trparsfix,idparsopt,idparsfix,idparsnoshift,pars2,brtsM,brtsS,missnumspec)
               }
           }
       }
   }
