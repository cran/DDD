dd_SR_simplex = function(trparsopt,idparsopt,trparsfix,idparsfix,idparsnoshift,pars2,brts,missnumspec)
{
...
for(i in 1:(numpar + 1))
{
   fv[i] = -dd_SR_loglik_choosepar(v[,i],trparsfix,idparsopt,idparsfix,idparsnoshift,pars2,brts,missnumspec)
}
...
{ 
   ...
   if(fxr < fv[1])
   {
       ...
       fxe = -dd_SR_loglik_choosepar(xe,trparsfix,idparsopt,idparsfix,idparsnoshift,pars2,brts,missnumspec)
       if(fxe < fxr)
       {
           v[,numpar + 1] = xe
           fv[numpar + 1] = fxe
       ...
       {      
           ...
           if(fxr < fv[numpar + 1])
           {
              ...
              fxco = -dd_SR_loglik_choosepar(xco,trparsfix,idparsopt,idparsfix,idparsnoshift,pars2,brts,missnumspec)
              if(fxco <= fxr)
              {
                 v[,numpar + 1] = xco
                 fv[numpar + 1] = fxco            
              ...
              fxci = -dd_SR_loglik_choosepar(xci,trparsfix,idparsopt,idparsfix,idparsnoshift,pars2,brts,missnumspec)
              if(fxci < fv[numpar + 1])
              {  
                 v[,numpar + 1] = xci
                 fv[numpar + 1] = fxci
           ...
           if(how == "shrink")
           {
               for(j in 2:(numpar + 1))
               {
                   ...
                   v[,j] = v[,1] + si * (v[,j] - v[,1])
                   fv[j] = -dd_SR_loglik_choosepar(v[,j],trparsfix,idparsopt,idparsfix,idparsnoshift,pars2,brts,missnumspec)
               }
           }
       }
   }
