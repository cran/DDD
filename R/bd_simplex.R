bd_simplex = function(trparsopt,idparsopt,trparsfix,idparsfix,pars2,brts,missnumspec)
{
...
for(i in 1:(numpar + 1))
{
   fv[i] = -bd_loglik_choosepar(v[,i],trparsfix,idparsopt,idparsfix,pars2,brts,missnumspec)
}
...
{ 
   ...
   fxr = -bd_loglik_choosepar(xr,trparsfix,idparsopt,idparsfix,pars2,brts,missnumspec)
 
   if(fxr < fv[1])
   {
       ## Calculate expansion point
       xe = (1 + rh * ch) * xbar - rh * ch * v[,numpar + 1]
       fxe = -bd_loglik_choosepar(xe,trparsfix,idparsopt,idparsfix,pars2,brts,missnumspec)
       if(fxe < fxr)
       {
           v[,numpar + 1] = xe
           fv[numpar + 1] = fxe
       ...
       {      
           ...
           if(fxr < fv[numpar + 1])
           {
              ...
              fxco = -bd_loglik_choosepar(xco,trparsfix,idparsopt,idparsfix,pars2,brts,missnumspec)
              if(fxco <= fxr)
              {
                 v[,numpar + 1] = xco
                 fv[numpar + 1] = fxco            
              ...
              fxci = -bd_loglik_choosepar(xci,trparsfix,idparsopt,idparsfix,pars2,brts,missnumspec)
              if(fxci < fv[numpar + 1])
              {  
                 v[,numpar + 1] = xci
                 fv[numpar + 1] = fxci
           ...
           if(how == "shrink")
           {
               for(j in 2:(numpar + 1))
               {
                   ...
                   v[,j] = v[,1] + si * (v[,j] - v[,1])
                   fv[j] = -bd_loglik_choosepar(v[,j],trparsfix,idparsopt,idparsfix,pars2,brts,missnumspec)
               }
           }
       }
   }
