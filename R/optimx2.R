optimx2 <- function(par, fn, gr=NULL, hess=NULL, lower=-Inf, upper=Inf, 
...
# RSE        cat("Maximizing -- use negfn and neggr\n")
	ufn <- negfn 
	ugr <- neggr
...
##JN          if ((ctrl$kkt || hessian) && (ans$conv != 9999)) {
##JN              if (ctrl$trace>0) cat("Compute Hessian approximation at finish of ",method[i],"\n")
##JN              if (!is.null(hess)){ # check if we have analytic hessian 
##JN                 nhatend<-try(hess(ans$par, ...), silent=TRUE)
##JN                 if (class(nhatend) != "try-error") {
##JN                    hessOK<-TRUE
##JN                    if (ctrl$maximize) nhatend<- (-1)*nhatend
##JN                 }
##JN              } else {
##JN                 if (is.null(gr)) {
##JN                     nhatend<-try(hessian(ufn, ans$par, ...), silent=TRUE) # change 20100711
##JN                 } else {
##JN                     nhatend<-try(jacobian(ugr,ans$par, ...), silent=TRUE) # change 20100711
##JN                 } # numerical hessian at "solution"
##JN                 if (class(nhatend) != "try-error") {
##JN                    hessOK<-TRUE
##JN                 }
##JN              } # end hessian calculation
##JN          } # end test if hessian computed
          ans$kkt1<-NA
          ans$kkt2<-NA
###          if((hessian || ctrl$kkt) && (ans$conv != 9999)) { # need to avoid test when method failed
##JN             if (ctrl$trace>0) cat("Compute gradient approximation at finish of ",method[i],"\n")
             gradOK<-FALSE
##JN             if (is.null(gr)) {
##JN                 ngatend<-try(grad(ufn, ans$par, ...), silent=TRUE) # change 20100711
##JN             } else {
##JN                 ngatend<-try(ugr(ans$par, ...), silent=TRUE) # Gradient at solution # change 20100711
##JN             }
##JN             if (class(ngatend) != "try-error") gradOK<-TRUE # 100215 had == rather than != here
##JN             if ( (! gradOK) && (ctrl$trace>0)) cat("Gradient computation failure!\n") 
             if (gradOK) {
                # test gradient
...
##JN                if (ctrl$dowarn) warning(warnstr)
                if (ctrl$trace>0) cat(warnstr,"\n") 
             }
###          } # end kkt test
          ans$systime <- time
