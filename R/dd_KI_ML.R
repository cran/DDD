dd_KI_ML = function(brtsM,brtsS,tsplit,initparsopt=c(0.5,0.1,2*(1+length(brtsM)),2*(1+length(brtsS)),max(c(brtsM,brtsS))/2),parsfix = NULL,idparsopt=c(1:3,6:7),idparsfix=NULL,idparsnoshift=(1:7)[c(-idparsopt,(-1)^(length(idparsfix) != 0) * idparsfix)],res=10*(1 + length(c(brtsM,brtsS))),ddmodel=1,missnumspec=0,cond = TRUE)
{
...
# - pars[1] = la = (initial) speciation rate in main clade
# - pars[2] = mu = extinction rate in main clade
# - pars[3] = K = carrying capacity in main clade
# - pars[4] = la2 = (initial) speciation rate in subclade
# - pars[5] = mu2 = extinction rate in subclade
# - pars[6] = K2 = carrying capacity in subclade
...
namepars = c("la_M","mu_M","K_M","la_S","muS","K_S","t_d")
...
out = optimx(trparsopt,dd_KI_loglik_choosepar,hess=NULL,method = "Nelder-Mead",control = list(maximize = TRUE,abstol = 1E-6,reltol = 1E-6,trace = 0,starttests = FALSE,kkt = FALSE),trparsfix = trparsfix,idparsopt = idparsopt,idparsfix = idparsfix,idparsnoshift = idparsnoshift,brtsM = brtsM, brtsS = brtsS, pars2 = c(res,ddmodel,cond,tsplit),missnumspec = missnumspec)
