...
rows = c(1:(lx - 1),1:lx,2:lx)
columns = c(2:lx,1:lx,1:(lx - 1))
xu = lavec[(2:(lx+1)) + kk - 1] * nn[(2:(lx+1)) + 2 * kk - 1]
xm = -(lavec[(2:(lx+1)) + kk] + muvec[(2:(lx+1)) + kk]) * nn[(2:(lx+1)) + kk]
xl = muvec[(2:(lx+1)) + kk + 1] * nn[(2:(lx+1)) + 1]
M = Matrix::sparseMatrix(i = rows,j = columns,x = c(xu,xm,xl))
